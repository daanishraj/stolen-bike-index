...
const Search = () => {
  ...
  const [filterByDateRange, setFilterByDateRange] = React.useState<[Date | null, Date | null]>([
    null,
    null,
  ]);
  const dateFilterIcon = <IconCalendar style={{ width: rem(18), height: rem(18) }} stroke={1.5} />;
  ...
  const getFiltersContent = () =>
    bikeSearchData.length > 0 ? (
      <Flex className={styles.filters} justify="center" columnGap="xl">
        <Button onClick={handleClickMunichOnly} radius="lg">
          Stolen in Munich
          <Pill className={styles.pill} ml="md" color="dark" size="xs">
            {munichBikeCountData}
          </Pill>
        </Button>
        <Button onClick={handleClickAll} radius="lg">
          Stolen everywhere
          <Pill className={styles.pill} ml="md" size="xs">
            {bikeCountData.stolen}
          </Pill>
        </Button>
